...
- `media/` contains the published media bytes, organized to mirror `data/` ownership: `profile/` for the portrait and favicon, `projects/<project-id>/` for project media, and `awards/`, `publications/`, and `patents/` for certificate files and publication project images under their owning record's `id` folder.
