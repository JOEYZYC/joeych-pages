...
export const homeSchema = z
  .object({
    ...
    portrait: localMediaFileSchema,
    hero_background: z.object({
      light: localMediaFileSchema,
      dark: localMediaFileSchema,
    }).strict().readonly(),
  })
  .strict()
  .readonly()
...
export const aboutSchema = z
  .object({
    page: pageMetaSchema,
    overview: z.array(localizedSchema).min(1).readonly(),
...
export const projectPageSchema = z.object({ page: pageMetaSchema }).strict().readonly()
...
export const techStackSchema = z
  .object({
    ...
    skills: z.array(z.object({
      id: recordIdSchema,
      title: localizedSchema,
      tags: z.array(skillTagSchema).min(1).refine(hasUniqueIds, "duplicate skill tag ID").readonly(),
    }).strict().readonly()).refine(hasUniqueIds, "duplicate skill group ID").readonly(),
  })
  .strict()
  .readonly()
