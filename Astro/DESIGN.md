...
- `ProjectExplorer` progressively enhances static source-order project details into compact sticky category, project-name, and honor facet filtering, cards, and per-project native dialogs. Every selected facet narrows the other controls to compatible records; an incompatible prior value is cleared rather than yielding an empty card set. Cards and dialogs appear only after every listener attaches; without JavaScript complete project details remain available.
